...
const requiredFiles = [
  ...
  'public/images/og-board-game-english-club.svg',
...
addCheck(
  'Homepage title is Japanese-local',
  index.includes('福岡市西区') && index.includes('英語ボードゲームサークル'),
  'The static shell targets Japanese local search intent.',
  'Update index.html title with Japanese local discovery language.',
);
addCheck('Homepage meta description exists', /<meta name="description" content="[^"]{50,180}"/.test(index), 'The static shell has a concise Japanese description.', 'Add a 50-180 character description in index.html.');
addCheck('Open Graph image is local', index.includes('/images/og-board-game-english-club.svg'), 'Social previews use project-owned artwork.', 'Replace external OG image URLs.');
