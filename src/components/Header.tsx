import { ChevronDown, Languages, Mail, Menu, X } from 'lucide-react';
...
const contactLink = 'mailto:ministarenglish@mail.com?subject=Board%20Game%20English%20Club%20Inquiry';
    ...
    <header className="nav-glass fixed inset-x-0 top-0 z-40 border-b border-[#efc779]/80">
      <div className="mx-auto flex h-[66px] w-full max-w-[1360px] items-center justify-between gap-4 px-5 md:px-8">
        ...
        <nav className="hidden flex-1 items-center justify-end gap-1.5 md:flex xl:gap-2">
          ...
          <a
            href={contactLink}
            className="inline-flex items-center gap-1 rounded-full bg-[#d87522] px-3 py-2 text-[11px] font-bold uppercase tracking-[0.05em] text-white shadow-sm transition hover:bg-[#bd5c24]"
          >
            <Mail size={13} />
            {language === 'ja' ? '相談' : 'Contact'}
          </a>
        </nav>
      ...
      {open && (
        <nav className="grid gap-1 border-t border-[#efc779] bg-[#fffdf8] p-5 sm:grid-cols-2 md:hidden">
          ...
          <a href={contactLink} className="border-b border-[#eadfce] px-2 py-3 text-left text-sm font-bold uppercase tracking-[0.12em] text-[#d87522]">
            {language === 'ja' ? '相談する' : 'Contact'}
          </a>
        </nav>
      )}
    </header>
