...
const silverCircleGamePicks = [
  {
    title: 'Blokus',
    en: 'A bright shape-placement game for simple table talk about space, color, corners, and careful choices.',
    ja: '色と形を置いていくゲームです。場所、色、角、次の一手について短い英語を使いやすいです。',
  },
  {
    title: 'Camel Up',
    en: 'A light camel-race game with surprise moments, guesses, laughter, and easy questions about risk.',
    ja: 'ラクダのレースを予想する明るいゲームです。驚き、予想、リスクについて自然に話せます。',
  },
  {
    title: 'Ecosystem',
    en: 'A calm nature card game for talking about animals, balance, placement, and why a card fits.',
    ja: '自然と動物のカードゲームです。置き場所、バランス、なぜそのカードを選ぶかを話せます。',
  },
  {
    title: 'Menara',
    en: 'A cooperative tower-building game where players ask for help, give advice, and celebrate steady teamwork.',
    ja: '協力して塔を作るゲームです。助けを求める、アドバイスする、協力を喜ぶ英語に向いています。',
  },
  {
    title: 'Sea Salt & Paper',
    en: 'A gentle card game with beautiful paper art, set collection, and small decisions about when to stop.',
    ja: '紙のアートが美しいカードゲームです。集める、待つ、やめるタイミングを短く話せます。',
  },
  {
    title: 'The Island',
    en: 'A dramatic escape game with boats, sea creatures, and funny table moments about safety and danger.',
    ja: '島から脱出するゲームです。船、安全、危険、助け合いについて会話が生まれやすいです。',
  },
] as const;
...
const copy = {
  en: {
    ...
    localDoorwayTitle: 'A gentle activity for seniors in Nishi-ku, Fukuoka',
    localDoorwayBody: 'Silver Circle is for retired adults and older residents who want a small local reason to go out, meet people, and enjoy simple English board games with Japanese support.',
    localDoorwayItems: [
      ['For seniors in Nishi-ku', 'A calm local activity for older adults who want conversation, light games, and a friendly table.'],
      ['For families searching for a place', 'A gentle first step for a parent or relative who may benefit from a regular reason to go out.'],
      ['For complete beginners', 'No English tests, no performance pressure, and no board game experience needed.'],
    ],
    pain: [
    ...
    faqs: [
      ...
      ['Is this a senior activity in Nishi-ku, Fukuoka?', 'Yes. Silver Circle is designed as a small local activity for seniors and retired adults in the Nishi-ku, Fukuoka area.'],
      ['Can a family member ask first?', 'Yes. Family members are welcome to ask questions by email before a parent or relative tries the table.'],
    ...
    footerAbout: 'Silver Circle is a local participation program from Board Game English Club - Fukuoka Chapter. It supports conversation, social connection, and joyful learning through English board games.',
  },
  ja: {
    ...
    proof2: '参加を通じて孤立を防ぐ',
    proof3: '少人数・徒歩圏内',
    localDoorwayTitle: '福岡市西区のシニア向け 少人数英語ボードゲームサークル',
    localDoorwayBody: 'Silver Circle は、退職後の新しい趣味、外出のきっかけ、ご近所での交流を探している方のための、やさしい地域活動です。英語が初めてでも、日本語サポートで安心して参加できます。',
    localDoorwayItems: [
      ['福岡市西区の高齢者向け', '静かで安心できる少人数テーブルです。地域で参加できる趣味活動を探している方に向いています。'],
      ['ご家族が探している場合にも', '親御さんの外出機会、会話の場、退職後の居場所を探しているご家族にも説明しやすい活動です。'],
      ['英語・ゲームが初めてでも大丈夫', 'テストなし、プレッシャーなし。日本語サポートと短い英語から始めます。'],
    ],
    pain: [
    ...
    faqs: [
      ...
      ['福岡市西区で高齢者が参加できる趣味活動ですか？', 'はい。Silver Circle は、福岡市西区周辺でシニア・退職後の方が参加しやすい少人数の地域活動として作っています。'],
      ['家族が先に問い合わせてもよいですか？', 'はい。親御さんやご家族の外出機会、会話の場を探している方からのご相談も歓迎です。'],
      ['認知症予防になりますか？', '医療行為や治療ではありません。ただし、会話、社会参加、考える遊びを通じて、楽しく頭を使う時間を作ります。'],
    ...
    footerAbout: 'Silver Circle は、Board Game English Club - Fukuoka Chapter による地域参加プログラムです。英語のボードゲームを通じて、会話、つながり、楽しい学びを支えます。',
  },
} as const;
...
export function SilverCircle({ onNavigate, language }: { onNavigate: (section: Section) => void; language: Language }) {
  ...
  const curatedGames = useMemo(() => {
    return silverCircleGamePicks.map((pick) => {
      const game = games.find((item) => normalizeTitle(item.title) === normalizeTitle(pick.title));
      return {
        ...pick,
        game,
      };
  ...
  const scrollToDetails = () => {
    document.getElementById('silver-details')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };
    ...
    <main className="silver-page min-h-screen pt-[66px]">
        ...
        <div className="silver-section grid items-center gap-10 lg:grid-cols-[0.95fr_1.05fr]">
          <div>
            ...
            <div className="mt-9 flex flex-wrap gap-4">
              ...
              <button type="button" onClick={scrollToDetails} className="rounded-full border border-[#fc849e] px-8 py-4 font-bold text-[#ea3159]">{t.details}</button>
            </div>
          ...
          <div className="relative">
            ...
            <span className="silver-card absolute -left-5 bottom-[-18px] rounded-2xl px-6 py-4 text-sm"><Brain className="mr-2 inline text-[#f33d67]" size={20} />{language === 'ja' ? '会話と脳の健康を' : 'Conversation and gentle thinking'}<br /><b className="ml-7 text-lg">{language === 'ja' ? '楽しく支える' : 'supported gently'}</b></span>
      ...
      <section className="bg-white">
        <div className="silver-section">
          <div className="mx-auto max-w-4xl text-center">
            <p className="text-sm font-bold text-[#ef4164]">{language === 'ja' ? '西区・福岡市 / 高齢者 趣味 / シニア サークル' : 'Nishi-ku, Fukuoka / senior activity / local community table'}</p>
            <h2 className="silver-title mt-4 text-4xl leading-tight">{local.localDoorwayTitle}</h2>
            <p className="mx-auto mt-5 max-w-3xl text-lg leading-9 text-[#5f5553]">{local.localDoorwayBody}</p>
          </div>
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {local.localDoorwayItems.map(([title, body]) => (
              <article key={title} className="silver-card rounded-2xl p-7">
                <h3 className="text-xl font-bold text-[#2f251e]">{title}</h3>
                <p className="mt-4 leading-7 text-[#665b59]">{body}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="silver-section">
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {curatedGames.map(({ title, en, ja, game }) => (
            <article key={title} className="silver-card overflow-hidden rounded-2xl">
              <div className="relative h-40 bg-[#f3e4db]">
                {game?.cover_image_url ? (
                  <img src={game.cover_image_url} alt="" className="h-full w-full object-cover" />
                ) : (
                  <div className="flex h-full items-center justify-center px-5 text-center text-xl font-bold text-[#ef4164]">{title}</div>
                )}
              </div>
              <div className="p-6">
                <h3 className="text-xl font-bold">{title}</h3>
                <p className="mt-4 text-sm leading-7 text-[#615755]">{language === 'ja' ? ja : en}</p>
                <p className="mt-5 text-sm text-[#726866]">
                  {game ? `${game.min_players}-${game.max_players} / ${game.duration_minutes}m` : language === 'ja' ? '少人数向け' : 'Small-table friendly'}
                </p>
              </div>
            </article>
          ))}
      ...
      <footer className="bg-[#201d1d] text-[#dad3d0]">
        <div className="mx-auto grid max-w-6xl gap-9 px-5 py-14 md:grid-cols-3 md:px-8">
          ...
          <div><h2 className="text-xl font-bold text-white">Board Game English Club</h2><p className="mt-5 leading-7">{language === 'ja' ? '福岡市西区を拠点に、英語とボードゲームを通じた学びのコミュニティを運営しています。' : 'Based in Nishi-ku, Fukuoka, we build learning communities through English and board games.'}</p></div>
        ...
        <p className="mx-auto max-w-6xl border-t border-[#443f3d] px-5 py-7 text-center text-sm text-[#8d8784] md:px-8">© 2026 Board Game English Club - Fukuoka Chapter / Silver Circle Program, Nishi-ku, Fukuoka</p>
      </footer>
    </main>
  );
}
