...
const agents: Agent[] = [
  { rank: 1, callsign: 'Member A', title: 'Community Helper', xp: 860, wins: 11, games: 24, deployment: 94, specialty: 'Explaining Choices', trend: '+120 XP' },
  { rank: 2, callsign: 'Member B', title: 'Table Regular', xp: 720, wins: 9, games: 21, deployment: 88, specialty: 'Asking Questions', trend: '+80 XP' },
  { rank: 3, callsign: 'Member C', title: 'Briefing User', xp: 645, wins: 8, games: 18, deployment: 83, specialty: 'Session Notes', trend: '+65 XP' },
  { rank: 4, callsign: 'Member D', title: 'Returning Player', xp: 510, wins: 6, games: 16, deployment: 76, specialty: 'Planning Turns', trend: '+40 XP' },
  { rank: 5, callsign: 'Member E', title: 'Phrase Collector', xp: 390, wins: 4, games: 13, deployment: 69, specialty: 'Useful Phrases', trend: '+30 XP' },
  { rank: 6, callsign: 'Member F', title: 'New Member', xp: 220, wins: 2, games: 9, deployment: 54, specialty: 'First Sessions', trend: '+20 XP' },
];
...
const scoring = [
  ...
  ['Briefing Used', '+20 XP', 'Use a briefing card or table play device.'],
...
const leaderboardTranslations = {
  en: {
    ...
    actions: [
      ['Build Table Play Device', 'play' as Section],
    ...
    rankHeaders: ['Rank', 'Member', 'XP', 'Sessions', 'Games', 'Growth'],
    tiers: rankTiers,
  },
  ja: {
        ...
        <section className="mt-8 grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <article className="reference-panel overflow-hidden">
            <div className="border-b border-[#f1d8a5] bg-[#fff8ea] px-6 py-4">
              <p className="eyebrow">{language === 'ja' ? 'Preview Example' : 'Preview Example'}</p>
              <h2 className="font-display mt-2 text-3xl tracking-wide text-[#bd5c24]">{language === 'ja' ? '参加例' : 'Example Member'}</h2>
            </div>
            <div className="p-6 text-center">
              ...
              <button onClick={() => onNavigate('play')} className="rule-button rule-button-primary mt-6 w-full justify-center py-3">
                <Sparkles size={14} /> {language === 'ja' ? 'Table Play を始める' : 'Start Table Play'}
              </button>
            </div>
          </article>
        </section>
        ...
        <section className="reference-panel mt-8 flex flex-wrap items-center justify-between gap-4 p-6">
          <div>
            <p className="font-display text-2xl tracking-wide text-[#bd5c24]">
              {language === 'ja' ? 'まずは一つの記録から' : 'Start With One Small Record'}
            </p>
            <p className="mt-1 text-xs text-[#70665b]">
              {language === 'ja'
                ? 'Table Play でゲーム、目標、質問を選び、最後にメンバープロフィールへ保存します。'
                : 'Use Table Play to choose the game, goal, and question, then save the final note to your Member Profile.'}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => onNavigate('profile')} className="rounded border border-[#ebba63] bg-white px-4 py-3 text-[11px] font-bold uppercase text-[#b85d1f]">
              {language === 'ja' ? 'プロフィールを見る' : 'View Member Profile'}
            </button>
            <button onClick={() => onNavigate('play')} className="rule-button rule-button-primary px-5 py-3">
              {language === 'ja' ? 'Table Play を開く' : 'Open Table Play'} <ArrowUpRight size={14} />
            </button>
          </div>
        </section>
      </div>
